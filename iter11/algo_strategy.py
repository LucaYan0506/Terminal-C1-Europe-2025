...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def predict_attack_side(self, game_state):
        ...
        Returns lists of attack directions: ['LEFT'], ['RIGHT'], or ['LEFT', 'RIGHT']
        Uses multiple analysis methods for more accurate prediction:
        1. Path analysis - checks where units from enemy spawn locations would exit
        2. Structure cost analysis - estimates enemy investment in each side
        3. Structural analysis - analyzes the pattern and types of enemy defenses
        4. Previous attack patterns - uses enemy_append_removal_unit to detect patterns
        """
        newAns = {
            -1: ['LEFT'],
            0: ['LEFT','RIGHT'],
        ...
        for left, right in zip(left_edges, right_edges):
            if left_exit_side == 0 and not game_state.contains_stationary_unit(left):
                ...
                if path and path[-1][1] >= game_state.game_map.HALF_ARENA:
                    for x, y in path[::-1]:
        ...
        # if both left + right edges exit to the same side or if exactly one edge does exit, we have a clear prediction
        ...
        # PART 2: STRUCTURE COST ANALYSIS
        ...
        # Price difference as a signal for investment in defense
        price_threshold = 10
        if left_price >= right_price + price_threshold:
            return newAns[1]  # Attack likely from right
        elif right_price >= left_price + price_threshold:
            return newAns[-1]  # Attack likely from left

        # PART 3: ANALYZE WALL REMOVALS AND ATTACK PATTERNS
        left_removals = 0
        right_removals = 0
        
        # Count recent wall removals (signals attack preparation)
        for (x, y) in self.enemy_append_removal_unit:
            if y >= 14:  # Near our territory
                if x < 14:
                    left_removals += 1
                else:
                    right_removals += 1
        
        # Strong signal from removal patterns
        if left_removals > right_removals + 2:
            return newAns[-1]  # Attack from LEFT more likely
        elif right_removals > left_removals + 2:
            return newAns[1]  # Attack from RIGHT more likely

        # PART 4: EDGE LINE STRUCTURAL ANALYSIS
        # Analyze the front line of enemy defenses
        ...
        for (lx, ly), (rx, ry) in zip(left_corner, right_corner):
            # Check left side
            if not game_state.contains_stationary_unit([lx, ly]):
                left_front_empty += 1
            else:
                left_struct = game_state.game_map[lx, ly][0]
                if left_struct.player_index == 1 and left_struct.upgraded:
                    left_upgraded += 1

            # Check right side
            if not game_state.contains_stationary_unit([rx, ry]):
                right_front_empty += 1
            else:
                right_struct = game_state.game_map[rx, ry][0]
                if right_struct.player_index == 1 and right_struct.upgraded:
                    right_upgraded += 1
        
        # More empty positions on one side indicates potential attack route
        empty_threshold = 3
        if left_front_empty > right_front_empty + empty_threshold:
            return newAns[-1]  # Attack from LEFT more likely
        elif right_front_empty > left_front_empty + empty_threshold:
            return newAns[1]  # Attack from RIGHT more likely
        
        # PART 5: UNIT TYPE ANALYSIS FOR PATTERN DETECTION
        # More turrets on one side generally indicates other side is the attack side
        turret_threshold = 2
        if left_turrets > right_turrets + turret_threshold:
            return newAns[1]  # Attack likely from RIGHT
        elif right_turrets > left_turrets + turret_threshold:
            return newAns[-1]  # Attack likely from LEFT

        # Support structures suggest defensive investment, so attack likely from other side
        support_threshold = 2
        if left_supports > right_supports + support_threshold:
            return newAns[1]  # Attack likely from RIGHT
        elif right_supports > left_supports + support_threshold:
            return newAns[-1]  # Attack likely from LEFT

        
        # If we still can't determine after all analyses, predict both sides
        return newAns[0]
