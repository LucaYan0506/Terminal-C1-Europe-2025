...
class AlgoStrategy(gamelib.AlgoCore):
    def __init__(self):
        ...
        self.firstLineWall = [[8,11],[7,12],[6,13],[5,13],[8,10],[9,9],[10,8],[24,10],[25,11],[26,13],[27,13],[23,9]]
        ...
        self.turret_locations = [[6,12],[6,11],[7,11],[4,11],[3,12],[26,12],[25,12]]
    ...
    def spawnInterceptor(self, game_state):
        ...
        if enemyMP >= base * 3:
            ...
            game_state.attempt_spawn(INTERCEPTOR, [1, 12], 4 + int(enemy_support_counter / 1.5))
        elif enemyMP >= base * 2:
            # game_state.attempt_spawn(INTERCEPTOR, [8, 5], 2)
            game_state.attempt_spawn(INTERCEPTOR, [1, 12], 2 + int(enemy_support_counter / 1.5))
        elif enemyMP >= base:
            game_state.attempt_spawn(INTERCEPTOR, [1, 12], 1 + int(enemy_support_counter / 1.5))
    ...
    def attack(self,game_state):       
        ...
        if self.path_was_blocked == False and enemyTowerCount > 0:# and self.portOpened:  
            game_state.attempt_spawn(DEMOLISHER, self.portToOpen, numberOfDemolisher)

        if self.path_was_blocked == False and enemyTowerCount == 0 and enemyWallCount > 0 and game_state.get_resource(MP) >= 9.0:
        ...
        if self.ideal_exit(game_state, final_position) and self.is_path_clear_of_enemy_walls(game_state,path) and game_state.get_resource(MP) >= 5:
            ## if path is blocked, send iterceptor to clean the path
            if self.path_was_blocked:
                # slow down SCOUT
                game_state.attempt_spawn(WALL, [13,1])
                game_state.attempt_spawn(WALL, [14,2])
                game_state.attempt_spawn(WALL, [15,3])
                game_state.attempt_spawn(INTERCEPTOR, self.portToOpen, 3)
            game_state.attempt_spawn(SCOUT, [14, 0], 1000)

        if self.ideal_exit(game_state, final_position) == False:
